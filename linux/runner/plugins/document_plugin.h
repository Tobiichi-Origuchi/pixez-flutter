...
class Document {
 private:
  ...
  static bool BuildDestinationPath(const std::string& file_name,
                                   std::string& out_path);
  static bool Save(const uint8_t* data, size_t length,
                   const std::string& full_path);
  static bool SaveFromPath(const std::string& source_path,
                           const std::string& full_path);
  static bool OpenSave(const uint8_t* data, size_t length,
  ...
  static void SaveTaskThread(GTask* task, gpointer source_object,
                             gpointer task_data, GCancellable* cancellable);
  static void SaveTaskDone(GObject* source_object, GAsyncResult* res,
                           gpointer user_data);
  static void SaveFromPathTaskThread(GTask* task, gpointer source_object,
                                     gpointer task_data,
                                     GCancellable* cancellable);
  static void SaveFromPathTaskDone(GObject* source_object, GAsyncResult* res,
                                   gpointer user_data);
